...
public class Config
{
    ...
    private static final ModConfigSpec.BooleanValue PVP_MODE = BUILDER
            ...
            .comment(" Enables hardness modifier to be calculated in block breaking speed")
            .define("pvp_mode", true);

    // Siege mode makes castleblocks unbreakable if not the original owner
    private static final ModConfigSpec.BooleanValue SIEGE_MODE = BUILDER
            .translation("castleblocks.configuration.siege_mode")
            .comment(" Makes Castle Block variants completely unbreakable if not the original placer")
            .define("siege_mode", true);

    private static final ModConfigSpec.DoubleValue MODIFIER = BUILDER
            .translation("castleblocks.configuration.modifier")
            .comment(" Hardness modifier for Castle Block variant break speeds")
            .defineInRange("hardness_modifier", 10.0, 0.0, 1000.0);
